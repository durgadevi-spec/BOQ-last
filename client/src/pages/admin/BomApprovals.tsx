...
export default function BomApprovals() {
    ...
    const toggleSelectAll = (checked: boolean) => {
        ...
        const approvableIds = visibleApprovals.filter(a => a.status === 'pending_approval' || a.status === 'submitted').map(a => a.id);
    ...
    const bulkApprove = async () => {
        ...
        try {
            for (const id of selectedIds) {
                await apiFetch(`/api/bom-approvals/${id}/approve`, { method: "POST" });
            }
    ...
    const bulkReject = async () => {
        ...
        try {
            for (const id of selectedIds) {
                await apiFetch(`/api/bom-approvals/${id}/reject`, {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ reason }),
                });
    ...
    const hasPendingActions = visibleApprovals.some(a => a.status === 'pending_approval' || a.status === 'submitted');
    const approvableCount = visibleApprovals.filter(a => a.status === 'pending_approval' || a.status === 'submitted').length;
                                                ...
                                                <TableHead className="w-[40px]">
                                                    <div className="flex items-center justify-center">
                                                        <input
                                                            ...
                                                            checked={approvableCount > 0 && selectedIds.length === approvableCount}
